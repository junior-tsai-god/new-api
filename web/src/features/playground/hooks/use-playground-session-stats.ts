...
export function usePlaygroundSessionStats(
  ...
  const query = useQuery({
    ...
    queryFn: () => getPlaygroundSessionStats(requestIds),
    enabled: Boolean(userId) && requestIds.length > 0,
    refetchInterval: (currentQuery) => {
      const stats = currentQuery.state.data
      if (
        stats?.settled !== false ||
        currentQuery.state.dataUpdateCount >= MAX_STATS_POLL_RESULTS
      ) {
        return false
      }
  ...
  return {
    stats: query.data,
    hasStatsError: query.isError,
    isLoadingStats: query.isPending && requestIds.length > 0,
    isSettlingStats: query.isFetching || query.data?.settled === false,
