...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	quota, clamp := common.QuotaFromDecimalChecked(decimal.NewFromInt(selectedProduct.Quota))
	...
	if selectedProduct.Price <= 0 || math.IsNaN(selectedProduct.Price) || math.IsInf(selectedProduct.Price, 0) {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "产品价格配置错误"})
		return
	}
	paymentCurrency, err := model.NormalizePaymentCurrency(selectedProduct.Currency)
	if err != nil || paymentCurrency == "" {
